...
internal static class NavmeshIpc {
	// 静态变量，用于跟踪是否已经输出过日志
	private static bool _hasLoggedInitSuccess = false;
	private static bool _hasLoggedPluginNotFound = false;
	...
	internal static Task<List<Vector3>>? Pathfind(Vector3 from, Vector3 to, bool fly = false) {
		if (_navPathfind == null) return Task.FromResult<List<Vector3>>(new List<Vector3>());

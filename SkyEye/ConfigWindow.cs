...
public class ConfigWindow : Window, IDisposable {
	...
	public override void Draw() {
		...
		ImGui.SameLine();
		if (ImGui.Button("reset")) Plugin.SetSpeed(1f);
		...
		if (ImGui.InputText("Territory id", ref _configuration.Overlay2DSpeedUpTerritory, 114514)) _configuration.Save();
		ImGui.Text("无视周边的挂壁亲友（用竖线|隔开）");
		if (ImGui.InputText("Friend id", ref _configuration.Overlay2DSpeedUpFriendly, 114514)) _configuration.Save();
		ImGui.Text("周围人数：" + _plu.OtherPlayer.Count);
		ImGui.Separator();
		ImGui.Text("NM开战时间喊话");
		...
		if (ImGui.Button("发送至喊话频道") && !string.IsNullOrWhiteSpace(_configuration.NmBattleTimeText)) {
			if (Plugin.ClientState.LocalPlayer == null ||  Plugin.ClientState.TerritoryType != 732 && Plugin.ClientState.TerritoryType != 763 && Plugin.ClientState.TerritoryType != 795 && Plugin.ClientState.TerritoryType != 827) return;
			_chat ??= new Chat();
